<script setup lang="ts">
import { AlertTriangle, CheckCircle2, Eye, EyeOff, KeyRound } from '@lucide/vue'
import { useAppToast } from '~/presentation/shared/composables/useAppToast'
import type { ApiResponse } from '~/presentation/shared/interfaces/api-response.interface'
import type { HttpClientError } from '~/presentation/shared/interfaces/http/http-client-error.interface'
import { resolveHttpErrorMessage } from '~/utils/http/resolve-http-error-message.util'
import { formatValidationMessage } from '~/utils/string/text-format.util'
import { usePasswordResetQuery } from '../composables/usePasswordResetQuery'
import { useResetPasswordMutation } from '../composables/usePasswordRecoveryMutations'

defineOptions({ name: 'ResetPasswordView' })

useHead({ title: 'Reiniciar contraseña · Sistema' })

type FieldKey = 'newPassword' | 'confirmPassword'

const route = useRoute()
const toast = useAppToast()
const resetPasswordMutation = useResetPasswordMutation()
const resetToken = computed(() => (typeof route.query.token === 'string' ? route.query.token : ''))
const passwordResetQuery = usePasswordResetQuery(resetToken)

const form = reactive<Record<FieldKey, string>>({
    newPassword: '',
    confirmPassword: '',
})
const fieldErrors = reactive<Record<FieldKey, string>>({
    newPassword: '',
    confirmPassword: '',
})
const formError = ref('')
const showPassword = ref(false)

const passwordRequirements = computed(() => [
    { label: '10 caracteres', valid: form.newPassword.length >= 10 },
    { label: 'Una mayúscula', valid: /[A-Z]/.test(form.newPassword) },
    { label: 'Una minúscula', valid: /[a-z]/.test(form.newPassword) },
    { label: 'Un número', valid: /[0-9]/.test(form.newPassword) },
    { label: 'Un símbolo', valid: /[^a-zA-Z0-9]/.test(form.newPassword) },
])
const tokenError = computed(() => {
    if (!resetToken.value) return 'El enlace de recuperación no incluye token.'
    if (resetToken.value.length < 32) return 'El enlace de recuperación está incompleto.'
    if (!passwordResetQuery.error.value) return ''
    return resolveHttpErrorMessage(
        passwordResetQuery.error.value,
        'El enlace expiró, fue utilizado o no es válido.',
    )
})
const canSubmit = computed(() => !!passwordResetQuery.data.value && !tokenError.value)

watch(
    () => form.newPassword,
    () => {
        fieldErrors.newPassword = ''
        formError.value = ''
    },
)
watch(
    () => form.confirmPassword,
    () => {
        fieldErrors.confirmPassword = ''
        formError.value = ''
    },
)

function validate() {
    fieldErrors.newPassword = ''
    fieldErrors.confirmPassword = ''
    formError.value = ''

    if (!canSubmit.value) {
        formError.value = tokenError.value || 'Espera mientras validamos el enlace.'
    }
    if (passwordRequirements.value.some(({ valid }) => !valid)) {
        fieldErrors.newPassword = 'La contraseña todavía no cumple todos los requisitos.'
    }
    if (form.confirmPassword !== form.newPassword) {
        fieldErrors.confirmPassword = 'Las contraseñas no coinciden.'
    }

    return !formError.value && !Object.values(fieldErrors).some(Boolean)
}

function applyApiValidation(error: unknown) {
    const details = (error as HttpClientError | undefined)?.details as ApiResponse<null> | undefined
    const message = details?.error?.fields?.newPassword?.[0]
    if (!message) return false

    fieldErrors.newPassword = formatValidationMessage(message)
    return true
}

async function submit() {
    if (!validate()) return

    try {
        await resetPasswordMutation.mutateAsync({
            resetToken: resetToken.value,
            newPassword: form.newPassword,
        })
        toast.success('Contraseña reiniciada correctamente')
        await navigateTo('/login')
    } catch (error) {
        if (applyApiValidation(error)) return
        formError.value = resolveHttpErrorMessage(error, 'No fue posible reiniciar la contraseña')
    }
}
</script>

<template>
    <section class="relative z-10 w-full max-w-[520px]">
        <div class="system-glass-panel rounded-lg p-6 md:p-8">
            <div class="text-center">
                <span
                    class="mx-auto flex size-12 items-center justify-center rounded-full bg-primary/15 text-primary"
                >
                    <KeyRound class="size-6" />
                </span>
                <h1 class="mt-4 font-display text-3xl font-semibold text-on-surface">
                    Reiniciar contraseña
                </h1>
                <p class="mt-2 text-sm leading-relaxed text-on-surface-variant">
                    Crea una contraseña nueva para volver a ingresar.
                </p>
            </div>

            <div
                v-if="resetToken && passwordResetQuery.isPending.value"
                class="mt-6 rounded border border-primary/25 bg-primary/5 px-3 py-2 text-xs text-on-surface-variant"
            >
                Validando enlace de recuperación...
            </div>
            <div
                v-else-if="passwordResetQuery.data.value"
                class="mt-6 flex items-start gap-2 rounded border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300"
            >
                <CheckCircle2 class="mt-0.5 size-4 shrink-0" />
                <span> Enlace válido para {{ passwordResetQuery.data.value.displayName }}. </span>
            </div>
            <div
                v-else-if="tokenError"
                class="mt-6 flex items-start gap-2 rounded border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive"
            >
                <AlertTriangle class="mt-0.5 size-4 shrink-0" />
                <span>{{ tokenError }}</span>
            </div>

            <form class="mt-7 space-y-5" novalidate @submit.prevent="submit">
                <div>
                    <UiLabel for="new-password" class="text-xs uppercase text-on-surface-variant">
                        Nueva contraseña
                    </UiLabel>
                    <div class="relative mt-2">
                        <KeyRound
                            class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-on-surface-variant"
                        />
                        <UiInput
                            id="new-password"
                            v-model="form.newPassword"
                            :type="showPassword ? 'text' : 'password'"
                            autocomplete="new-password"
                            class="h-11 bg-surface/70 pl-10 pr-10"
                            :aria-invalid="!!fieldErrors.newPassword"
                            :disabled="!canSubmit"
                        />
                        <button
                            type="button"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-primary disabled:opacity-50"
                            :aria-label="showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'"
                            :disabled="!canSubmit"
                            @click="showPassword = !showPassword"
                        >
                            <EyeOff v-if="showPassword" class="size-4" />
                            <Eye v-else class="size-4" />
                        </button>
                    </div>
                    <p v-if="fieldErrors.newPassword" class="mt-1 text-xs text-destructive">
                        {{ fieldErrors.newPassword }}
                    </p>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <span
                            v-for="requirement in passwordRequirements"
                            :key="requirement.label"
                            class="inline-flex items-center gap-1 rounded border px-2 py-1 text-[10px]"
                            :class="
                                requirement.valid
                                    ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300'
                                    : 'border-outline-variant text-on-surface-variant'
                            "
                        >
                            <CheckCircle2 class="size-3" /> {{ requirement.label }}
                        </span>
                    </div>
                </div>

                <div>
                    <UiLabel
                        for="confirm-password"
                        class="text-xs uppercase text-on-surface-variant"
                    >
                        Confirmar nueva contraseña
                    </UiLabel>
                    <UiInput
                        id="confirm-password"
                        v-model="form.confirmPassword"
                        :type="showPassword ? 'text' : 'password'"
                        autocomplete="new-password"
                        class="mt-2 h-11 bg-surface/70"
                        :aria-invalid="!!fieldErrors.confirmPassword"
                        :disabled="!canSubmit"
                    />
                    <p v-if="fieldErrors.confirmPassword" class="mt-1 text-xs text-destructive">
                        {{ fieldErrors.confirmPassword }}
                    </p>
                </div>

                <p
                    v-if="formError"
                    role="alert"
                    class="rounded border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive"
                >
                    {{ formError }}
                </p>

                <UiButton
                    type="submit"
                    class="h-11 w-full uppercase"
                    :loading="resetPasswordMutation.isPending.value"
                    :disabled="!canSubmit"
                >
                    Guardar contraseña
                </UiButton>
            </form>

            <NuxtLink
                to="/login"
                class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline"
            >
                Volver al login
            </NuxtLink>
        </div>
    </section>
</template>
